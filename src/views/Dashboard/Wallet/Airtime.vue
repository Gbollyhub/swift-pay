  ...
  <div class="wallet-modal">
    <div class="wallet-modal-div">
      ...
      <div v-show="walletView">
        ...
        <p style="font-size: 13px; color: #bd161c" v-show="erroralert">
          Invalid PIN!
        </p>
        <p style="font-size: 13px; color: #bd161c" v-show="erroralert2">
          No Sufficient Fund! Fund Account
        </p>
...
export default {
  ...
  methods: {
    ...
    async buyAirtime() {
      ...
      const network = this.user.networkChoosen;
      ...
      const resData = {
        ...
        remarks: "-",
        details: `Airtime Purchase - Network - ${network} - ${phone_number}`,
        amount: amount,
      ...
      if (this.getUser.account_pin == pin) {
        if (this.getUser.account_balance > this.user.amount) {
          try {
            ...
            const transaction3 = await axios.patch(
              `${url}/update-account`,
              resData2,
              { headers: { Authorization: `Bearer ${token}` } },
              { timeout: 10 }
    ...
    addToBulkRecipient() {
      if (this.user.name < 5)
        return this.setError("Valid Wallet ID and Name is required");
      this.bulkRecipient.push({
    ...
    addToMultipleBulkRecipient() {
      if (this.user.name < 5)
        return this.setError("Valid Wallet ID and Name is required");
      if (Number(this.user.amount) < 1)
        return this.setError("Valid Amount is required");
      let obj = {
