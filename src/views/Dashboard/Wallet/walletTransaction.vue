<template>
  <div class="wallet-transaction">
    <v-card class="wallet-transaction-card" flat>
      <v-row>
        <v-col
          @click="toWallet2Wallet"
          class="wallet-transaction-card-col"
          cols="12"
          sm="6"
          md="4"
        >
          <div class="wallet-transaction-card-div">
            <v-icon style="font-size: 60px; margin-bottom: 10px"
              >mdi-wallet</v-icon
            >
      ...
      <div v-if="getTransactions.length > 0">
        <table>
          ...
          <tbody
            v-for="(transaction, index) in getTransactions.slice(0, 5)"
            :key="transaction._id"
          >
            <tr>
              <td>{{ index }}</td>
              ...
              <td>
                ...
                <br />
                <span
                  style="margin-left: 38px; color: #ababab; font-size: 13px"
                  >{{ transaction.createdAt }}</span
                >
              </td>
              <td :style="{ textAlign: alignRight }">
                <span
                  :style="{
                    color:
                      transaction.transaction_type == 'Debit'
