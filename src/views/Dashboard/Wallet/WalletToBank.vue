<template>
	<div class="wallet-modal" >
			...
			<div v-show="walletView">
				...
				<p style="font-size:13px;color:#BD161C" v-show="erroralert2">No Sufficient Fund! Fund Account</p>
...
export default {
	...
	data() {
		return {
            ...
            transferType: 'single',
            bank: ['Alat','Access', 'Gtb', 'Wema', 'Zenith', 'Stanbic', 'Fidelity', 'Uba'],
			user: {
				amount: '',
				account_number: '',
				remark: '',
	...
	methods: {
		  ...
		  const resData2 = {
           ...
           if(this.getUser.account_pin == pin){
			   if( this.getUser.account_balance > this.user.amount){
                ...
                const transaction3 = await axios.patch(`${url}/update-account`, resData2,{ headers: { 'Authorization': `Bearer ${token}` } }, { timeout: 10 })
		...
		closeModal() {
			this.user.amount = this.user.account_number = this.user.walletID = this.user.pin = '';
			 this.successView = false,
            this.failedView = false,
            this.walletView = true
